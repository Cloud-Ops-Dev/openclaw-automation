...
export class FastmailClient {
  ...
  async listDrafts(limit: number = 50): Promise<{ drafts: Email[]; total: number }> {
    console.error('📋 Listing drafts...');
    ...
    const [, getResult] = getResponse.methodResponses[0];
    console.error(`✅ Found ${getResult.list.length} drafts`);

    return { drafts: getResult.list, total };
  }

  async sendDraft(draftId: string): Promise<{ emailId: string; sentAt: string }> {
    console.error('📤 Sending draft:', draftId);

    const identityId = await this.getPrimaryIdentity();

    // Get the draft to extract recipient info
    const draft = await this.getEmail(draftId);
    if (!draft) {
    ...
    const rcptTo = [
      ...(draft.to || []).map(t => ({ email: t.email })),
      ...(draft.cc || []).map(t => ({ email: t.email })),
      ...(draft.bcc || []).map(t => ({ email: t.email }))
    ];
      ...
      methodCalls: [
        ...
        ['EmailSubmission/set', {
          ...
          onSuccessUpdateEmail: {
            '#sendIt': {
              'mailboxIds': null  // Will be moved to Sent by server
            }
          },
          create: {
            'sendIt': {
    ...
    if (!sendResult || !sendResult.created || !sendResult.created.sendIt) {
      throw new Error(`Failed to send draft: ${JSON.stringify(sendResult)}`);
    }

    const sentAt = sendResult.created.sendIt.sendAt;
    console.error('✅ Draft sent at:', sentAt);

    return { emailId: draftId, sentAt };
  }

  async deleteDraft(draftId: string): Promise<void> {
    console.error('🗑️ Deleting draft:', draftId);
